...
###################################### debut du code propre à l'images

def dreamify(image,style,contenu):
    """contenu entre 0.001 et 0.1"""

    imname = image+".jpg"
    photo = plt.imread(imname)

    rawim, photo = prep_image(photo)
    art = plt.imread(style+'.jpg')
    ...
    losses.append(contenu * content_loss(photo_features, gen_features, 'conv4_2'))# on choisit ici l'importance du contenu
    ...
    for i in range(5):
        ...
        plt.imsave(image+'_'+style+'_'+contenu+'_'+str(i)+'.png',deprocess(xs[i]))
    #plt.tight_layout()


    #plt.figure(figsize=(8,8))
    plt.imsave(image+'_'+style+'_'+contenu+'_'+'final.png',deprocess(xs[-1]))
    print start-time.time()


for image in [beatles,charge,windows,porte]:
    for style in [cubism,vangogh,vague]:
        for contenu in [0.1,0.01,0.001]:
            dreamify(image,style,contenu)
